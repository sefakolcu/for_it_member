...
class DataLoaderThread(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            self.data_loaded.emit(ships_data)
...
class MyApp(QtWidgets.QMainWindow):
    ...
    def create_table_search(self):
        ...
        self.table_widget_srch.setHorizontalHeaderLabels(["IMO Number", "ETA", "Ship Name", "Type", "Port", "Name of Company", "Company Role", "Adress", "Port Loco"])
    ...
    def export_to_excel(self):
        ...
        df = pd.DataFrame(data, columns=["IMO Number", "ETA", "Ship Name", "Type", "Port", "Name of Company", "Company Role", "Adress", "Port Loco"])
    ...
    def on_combobox_changed2(self):
        selected_country = self.comboBox2.currentText()
        
        if selected_country == "Turkey":
            port_loco_prefixes = ["TR", "RO", "UA", "BG", "GE"]
        else:
            port_loco_prefixes = ["GR"]
        
        for row in range(self.table_widget_srch.rowCount()):
            port_loco = self.table_widget_srch.item(row, 8).text()
            if any(port_loco.startswith(prefix) for prefix in port_loco_prefixes):
                self.table_widget_srch.setRowHidden(row, False)
            else:
                self.table_widget_srch.setRowHidden(row, True)
    ...
    def load_data_from_api(self):
        ...
        print("yes internet")
        GET_URL = "http://ip_adress/get_combined/"
        headers = {
            "username": "sefakusr",
            "apikey": "api_key",
            "usrfingerprint": "user_finger_print"
        }

        self.table_widget_srch.setRowCount(0)  # Clear the table before loading data

        # Create and start the data loader thread
        self.data_loader_thread = DataLoaderThread(GET_URL, headers)
        self.data_loader_thread.data_loaded.connect(self.update_table)  # Connect signal to slot
        self.data_loader_thread.error_occurred.connect(self.handle_error)  # Connect error signal to slot
        self.data_loader_thread.start()

    def update_table(self, ships_data):
        column_keys = ["imo_number", "eta", "ship_name", "comp_type", "port", "name_of_company", "company_role", "address", "port_loco", "ship_val_code", "today_date"]

        for ship in ships_data:
            row_position = self.table_widget_srch.rowCount()
            self.table_widget_srch.insertRow(row_position)

            row_data = [ship.get(key, "-") if ship.get(key) else "-" for key in column_keys]
            row_data.append("Not Arrived")  # Add extra column data
    ...
    def handle_error(self):
        QtWidgets.QMessageBox.critical(self, "Error", "Failed to load data from the server.")
    ...
    def validate_license(self):
        ...
        if check_internet():
            if len(rows) < 1:
                GET_URL = "http://ip_adress/check_license/"
                headers = {
